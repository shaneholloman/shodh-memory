...
**Amendments required to Tasks 1–5: 17** (numbered list at end — 2 for Task 1, 6 for Task 2, 5 for Task 3, 1 for Task 4, 3 for Task 5).
...
**Verdict on the plan's named list:** decay sweeps (`decay.rs` has zero direct RocksDB writes — it mutates through `storage.update`/`persist_access_updates`), `mark_forgotten_*`, `remove_from_indices`, index rebuilds (`consolidation.rs::rebuild_index` rebuilds the *vector* index from `default` CF records; no CF-level operation), canonicalization/consolidation, `repair_index`, and `migration.rs` record rewriting — **none writes to a CF it does not name explicitly.** Adding `CF_OPLOG` is invisible to all of them. **SAFE.**
...
- The plan never states the oplog's write durability. `WriteMode::default()` (`storage.rs:1191`) is async unless `SHODH_WRITE_MODE=sync`. For an audit log, appends should be `set_sync(true)` regardless of the memory write mode — the precedent already exists at `storage.rs:2741` and `:2795`, where the forget sweeps force `write_opts.set_sync(true)` for integrity-critical writes.
...
3. **(Finding A, blocking)** In the *same commit* that adds the `CF_OPLOG` descriptor to `storage.rs:1175-1185`, add `ColumnFamilyDescriptor::new(CF_OPLOG, RocksOptions::default())` to the `cfs` vec at `migration.rs:324-327`. Splitting these across commits leaves `shodh migrate` broken on `main`.
...
6. **(Finding H)** Extend `MemoryStorage::flush` (`storage.rs:3159-3162`) with a `flush_cf_opt` for the oplog CF, and add `CF_OPLOG` to `rocksdb_memory_breakdown`'s array (`storage.rs:3129`). Force `write_opts.set_sync(true)` on oplog appends regardless of `WriteMode` — precedent at `storage.rs:2741`/`:2795`.
...
9. **(Finding B)** Mount `capture_middleware` **inside `router.rs::build_protected_routes`** (at `router.rs:474`), not at the `server.rs:261` call site. This covers HTTP *and* local IPC (`server.rs:339`, enabled by default per `server.rs:298-305`) and makes "inside auth" structural. Record in the code comment that `.layer()` order is inner-first: last-applied is outermost.
...
11. **(Finding C)** Identity-less requests: increment the loud counter and **drop the record**. Do **not** append under a synthetic user — `get_user_memory` (`state.rs:1242`) creates the user directory and a full `MemorySystem` on miss (with up to ~750 ms of retry sleeps at `:1288-1296`). Add a cache-only accessor to `MultiUserMemoryManager` (`user_memories.get()` is private; `cached_user_memories()` at `state.rs:1627` returns all users) and use it. This replaces the plan's `identity:unset` / `user unknown` rule.
